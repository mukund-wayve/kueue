...
func TestAPIs(t *testing.T) {
	util.RunE2ESuite(t, "End To End Logical TAS MultiKueue Suite")
}
...
var _ = ginkgo.BeforeSuite(func() {
	...
	managerRules := logicalTASManagerRules(ctx)
	...
	expectLogicalTASSpikeEnabled()
})

func expectLogicalTASSpikeEnabled() {
	ginkgo.GinkgoHelper()
	...
	gomega.Eventually(func(g gomega.Gomega) {
		...
		for _, c := range deploy.Spec.Template.Spec.Containers {
			...
			for _, env := range c.Env {
				if env.Name == "KUEUE_LOGICAL_TAS_SPIKE" && env.Value == "true" {
					found = true
					break
				}
			}
		}
		g.Expect(found).To(gomega.BeTrue(), "manager should have KUEUE_LOGICAL_TAS_SPIKE=true")
	}, util.LongTimeout, util.Interval).Should(gomega.Succeed())
}

func logicalTASManagerRules(ctx context.Context) []rbacv1.PolicyRule {
