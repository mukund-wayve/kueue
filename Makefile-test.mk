...
.PHONY: test-multikueue-e2e-logicaltas
test-multikueue-e2e-logicaltas: E2E_NPROCS := 1
test-multikueue-e2e-logicaltas: setup-e2e-env run-test-multikueue-e2e-logicaltas-$(E2E_KIND_VERSION:kindest/node:v%=%) ## Run the Logical TAS spike MultiKueue e2e test suite.
...
run-test-multikueue-e2e-logicaltas-%: K8S_VERSION = $(@:run-test-multikueue-e2e-logicaltas-%=%)
run-test-multikueue-e2e-logicaltas-%:
	@echo Running logical-TAS multikueue e2e for k8s ${K8S_VERSION}
	E2E_KIND_VERSION="kindest/node:v$(K8S_VERSION)" KIND_CLUSTER_NAME=$(KIND_CLUSTER_NAME) \
		ARTIFACTS="$(ARTIFACTS)/$@" IMAGE_TAG=$(IMAGE_TAG) GINKGO_ARGS="$(E2E_GINKGO_ARGS)" \
		E2E_MODE=$(E2E_MODE) \
		E2E_SKIP_REINSTALL=$(E2E_SKIP_REINSTALL) \
		E2E_ENFORCE_OPERATOR_UPDATE=$(E2E_ENFORCE_OPERATOR_UPDATE) \
		E2E_TARGET_FOLDER="multikueue/logicaltas" \
		E2E_CONFIG_FOLDER="multikueue/logicaltas" \
		TEST_LOG_LEVEL=$(TEST_LOG_LEVEL) \
		E2E_USE_HELM=$(E2E_USE_HELM) \
		./hack/testing/e2e-multikueue-test.sh
